...
export default function ReferralSection() {
  ...
  const handleGenerate = (e: React.FormEvent) => {
    ...
    setRefCode(generateCode());
  };

  const refLink = refCode ? `https://servelinkapp.com?ref=${refCode}` : "";
  ...
  const shareWhatsApp = () => {
    const text = encodeURIComponent(
      fr
        ? `Rejoins ServeLink ! La nouvelle app de services \u00e0 domicile au Cameroun. ${refLink}`
        : `Join ServeLink! The new home services app in Cameroon. ${refLink}`
    );
  ...
  const shareTwitter = () => {
    const text = encodeURIComponent(
      fr ? "Rejoins ServeLink \u2014 services \u00e0 domicile au Cameroun !" : "Join ServeLink \u2014 home services in Cameroon!"
    );
            ...
            {!refCode ? (
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-center gap-3">
                  <button onClick={shareWhatsApp} className="w-10 h-10 rounded-full bg-[#25D366] flex items-center justify-center text-white hover:scale-110 transition-transform" aria-label="WhatsApp">
